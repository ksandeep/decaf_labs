...
Type* MethodInvocation::typeinfer() {
   ...
   while(gamma!=NULL)
   {
   	...
   	for(i=members->begin();i!=members->end();i++)
   	{
	   	if((*i)->kind() == METHOD_ENTITY && (strcmp((*i)->name(),name)==0))
	   	{
		   	list<Entity *> *formal_params = ((MethodEntity*)*i)->formal_params();
		   	if(same_args(args,formal_params) && (((MethodEntity*)*i)->visibility_flag() || (curr_method && curr_class && (curr_class == gamma))) && (((MethodEntity*)*i)->static_flag() == static_flag))
			   	return ((MethodEntity *)*i)->return_type();
	
	   	}
   	}

	beta = gamma;
	gamma = beta->superclass();
   }

	error->type_error(this->lineno(),"undeclared method",name);
...
Type* NewInstance::typeinfer() {
   ...
   ClassEntity *gamma = this->class_entity();
   	...
   	for(i=members->begin();i!=members->end();i++)
   	{
	   	...
	   	{
		   	list<Entity *> *formal_params = ((MethodEntity*)*i)->formal_params();
		   	if(same_args(args,formal_params) && (curr_method && curr_class && (curr_class == gamma)))
