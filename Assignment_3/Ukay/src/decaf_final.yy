...
MethodDecl:
	  MethodHead TOK_OPEN_PAREN { global_symtab->enter_block(); is_func_enter = true;}FormalsOpt TOK_CLOSE_PAREN Block 
		{
			$$ = new MethodEntity($1.name,$1.bool_arr_2[0],$1.bool_arr_2[1],$1.type,$4,$6);
			is_func_enter = false;
...
FormalParam: {is_formal_param = true; } Type { t = $2; } Variable {is_formal_param = false; }	
		{
			/*
			bool b;
			Entity *e = global_symtab->find_entity($4->name(),VARIABLE_ENTITY, &b);
			if (e)
			{
					printf("Duplicate formal parameter: %s\n",$4->name());
					yyerror("Duplicate formal parameter ");
					$$ = NULL;
			}
			else
			{
...
Block:	  TOK_OPEN_BRACE {if(!is_func_enter)global_symtab->enter_block();}  StmtStar TOK_CLOSE_BRACE 	{ $$ = new BlockStatement($3); global_symtab->leave_block();}

	  ;

StmtStar:
	  Stmt StmtStar 		{ $$ = $2; $$->push_back($1); }
	| 				{ $$ = new list<Statement *>(); }
	;
