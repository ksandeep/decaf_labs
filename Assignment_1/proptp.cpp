...
//Symbol Queue for processing the incoming symbols...
queue<symbol> symQ;
...
/* Main functions reads the input string, validates the syntax and if the syntax is valid computes the consistency
 by calling different functions.. the steps are repeated as long as the incomings strings are consistent */
...
int main()
{
	...
	while(consistent)
	{
		...
		cout<<"- ";
		getline(cin,input);
		//check if the line terminates with '.' 
		...
		{
			valid = FALSE;
			//do a bit of pre-processing here... 
			preprocessed = preprocess(input);
			if (debug)
				cout<<"preprocessed = "<<preprocessed<<endl;
			//if the preprocessing step suceeded then validate the input...
			if(preprocessed)
			{
				if (debug)
			...
			if(valid)
			{
				...
				//and check if the expression is consistent witht the previous ones. 
...
/* Validate syntax reads a string and checks for all kind of validation errors. 
which includes 
1. unwanted spaces between identifiers.
2. invalid usage of brackets
3. improper usage of binary operators & unary operators
...
int validate_syntax(string str)
{
	...
	//check if no. of opening brackets is equal to no. of closing brackets...
	...
	// validate each of the expressions inside the brackets. 
	...
	//validate the whole string after the bracketted expressions are validated...
...
//removes the white spaces at the right end
string trim_right(const string &source , const string& t = " ")
{
	string str = source;
	return str.erase( str.find_last_not_of(t) + 1);
}
//removes the white spaces at the left end
...
//removes the white spaces at both ends
...
int check_validity(string str)
{
	...
	//if the operator is open brackets...
	//never comes to this location... 
	else if(str[index] == '(')
	{
...
// This function returns an integral precedence value corresponding to a terminal symbol passed as a string
// A higher value denotes a higher priority
// Returns -1 in case any string apart from the list of valid terminal symobls is passed
